...
function init(){
    ...
    for(let cell of cells){
        cell.addEventListener("click", handleClickShowImage);
    }
}
...
let compareArray = []
    ...
    function handleClickShowImage(ev){ // on récupère l'évènement au travers du paramètre ev
    ...
    //fonction de comparaison de deux images
    function compareImage(id1, id2){
        ...
        if (target1.firstChild.src != target2.firstChild.src){
            ...
            winGameTest(); // On appelle la fonction qui va tester si toutes les paires on été trouvées
        }
    
    }

    function winGameTest(){
        let images = document.getElementsByTagName('img')
        if (images.length == 28 ){
           alert("Bravoooooo !!!! Vous avez gagnez !!! En " + minutesElement.value + "mn");
